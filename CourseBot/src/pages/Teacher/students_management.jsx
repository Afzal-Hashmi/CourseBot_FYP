...
const StudentsManagement = () => {
    ...
    const fetchStudents = async () => {
      try {
        ...
        if (!token || role !== "teacher") {
          setMessage("Unauthorized access. Please log in as a teacher.");
          setLoading(false);
          navigate("/")
        }
  ...
  const handleRemove = async (enrollmentId, courseId) => {
    ...
    const token = Cookies.get("token");
    const response = await fetch("http://localhost:8000/teacher/remove-student", {
      method: "DELETE",
      headers: {
    ...
    if (response.ok) {
      ...
      console.log(res)
      if (!res.succeeded) {
        setMessage(res.message || "Failed to remove student.");
        return;
      }
      console.log("Student removed successfully");
      setStudentsData((prev) =>
        prev.filter(
          (student) => student.enrollment_id !== enrollmentId && student.course_id !== courseId
        )
      );
