...
class TeacherService:
        ...
        try:
            ...
            if res.status == 201:
                ...
                if db_record and db_record.get("content_id"):
                    try:
                        await self.teacher_repo.delete_content_repo(current_user,db_record.get("content_id"))
    ...
    async def delete_content_service(self, content_id: int, current_user:dict):
        try:
            response = await self.teacher_repo.delete_content_repo(current_user, content_id)
    ...
    async def remove_student_service(self, enrollment_id: int, course_id: int):
        try:
            response = await self.teacher_repo.remove_student_repo(enrollment_id, course_id)
            if not response:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
    ...
    async def upload_video(self, file: UploadFile):
        try:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"Could not save video file: {file_error}"
                )
