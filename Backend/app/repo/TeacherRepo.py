...
class TeacherRepository:
    ...
    async def fetch_courses_repo(self, current_user: dict):
        try:
            result = await self.db.execute(
                select(Course).filter(Course.teacher_id == current_user.get("id"))
            )
    ...
    async def delete_course_repo(self, course_id: int, current_user: dict):
        ...
        await self.db.execute(delete(ChatID).where(ChatID.course_id == course_id))
        await self.db.execute(
            delete(CourseContent).where(CourseContent.course_id == course_id)
        )
        await self.db.execute(
            delete(Enrollment).where(Enrollment.course_id == course_id)
        )
        await self.db.execute(
            delete(CourseFeedback).where(CourseFeedback.course_id == course_id)
        )
        await self.db.execute(
            delete(Course).where(
                Course.course_id == course_id,
    ...
    async def create_course_repo(self, form_Data: course_schema, current_user: dict):
        try:
            ...
            result = Course(
                **course, course_status="Draft", teacher_id=current_user.get("id")
            )
            self.db.add(result)
            await self.db.commit()
    ...
    async def fetch_feedback_repo(self, current_user: dict):
        try:
            result = await self.db.execute(
                ...
                .options(
                    selectinload(Course.course_feedback).selectinload(
                    ...
                    feedbacks_with_course_info.append(
                        {
                            ...
                            "student_name": (
                                f"{feedback.student.firstName} {feedback.student.lastName}"
                                if feedback.student
                                else "Anonymous"
                            ),
                            "student_email": (
                            ...
                            "student_profilePicture": (
                                feedback.student.profilePicture
                                if feedback.student
                                else "Anonymous"
                            ),
                        }
    ...
    async def upload_content_repo(
        self, form_data: course_content_schema, url: str = None
    ):
        try:
    ...
    async def get_content_repo(self, course_id: int):
        try:
            result = await self.db.execute(
                ...
                .options(selectinload(CourseContent.courses))
            )
    ...
    async def delete_content_repo(self, content_id: int):
        try:
            # Execute the delete query
            response = await self.db.execute(
                select(CourseContent).where(CourseContent.content_id == content_id)
            )
            result = await self.db.execute(
    ...
    async def delete_with_url_repo(self, url: str, current_user: dict):
        try:
            response = await self.db.execute(
                select(CourseContent).where(CourseContent.content_url == url)
            )
            result = await self.db.execute(
                delete(CourseContent).where(
    ...
    async def get_chat_id_repo(self, course_id: int, current_user: dict):
        try:
            result = await self.db.execute(
                select(ChatID).where(
                    ChatID.course_id == course_id,
                    ChatID.user_id == current_user.get("id"),
                )
            )
            contents = result.scalars().all()
            return contents
    ...
    async def update_teacher_info(
        self, teacher_id: int, update_data: dict, current_user: dict
    ):
        try:
            if (
                str(current_user.get("id")) != str(teacher_id)
                or current_user.get("role") != "teacher"
            ):
                raise HTTPException(
    ...
    async def edit_profile_repo(
        self, teacher_id: int, form_data: dict, current_user: dict
    ):
        try:
            user = await self.db.execute(
                select(User).where(
                    User.id == teacher_id, User.id == current_user.get("id")
                )
            )
