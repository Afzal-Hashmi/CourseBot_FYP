...
from ...schemas.teacherSchema import (
...
async def create_course_router(
    ...
    return await teacher_controller.create_course_controller(
        form_Data, current_user, file
    )
...
async def fetch_feedback_router(
    current_user: dict = Depends(get_current_user),
    teacher_controller: TeacherController = Depends(TeacherController),
):
    if not current_user:
...
async def upload_content(
    ...
    try:
        response = await teacher_controller.upload_content_controller(
            form_data, file, current_user
        )
        return response
...
async def ask_question_from_video(
    ...
    try:
        ...
        if not payload.chat_id:
            await teacher_controller.save_chat_id(
                payload.question,
                vectara_response.get("chat_id"),
                course_id,
                current_user,
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to connect to Vectara: {str(e)}"
        )
    except Exception as e:
